...
def apply_sepia(img):
    ...
    return ImageOps.colorize(gray, "#2b1b12", "#f2d3a0")
...
def image_to_bytes(img, fmt="JPEG", quality=85):
    buffer = io.BytesIO()
    save_img = img if fmt.upper() != "JPEG" else ensure_rgb(img)
    if fmt.upper() == "JPEG":
        save_img.save(buffer, format=fmt, quality=quality)
    else:
    ...
    if uploaded_image_file is not None:
        try:
            ...
            with control_col:
                ...
                crop_left = st.number_input("Left Cut (px)", min_value=0, max_value=max(orig_w - 1, 0), value=0)
                crop_top = st.number_input("Top Cut (px)", min_value=0, max_value=max(orig_h - 1, 0), value=0)
            ...
            with display_col:
                st.subheader("📋 Studio Pipeline Monitoring")

                try:
                    working_img = original_img.crop((crop_left, crop_top, crop_right, crop_bottom))
                    working_img = working_img.resize((int(resize_w), int(resize_h)), Image.Resampling.LANCZOS)
...
# =====================================================================
    ...
    with qr_pipeline[2]:
        ...
        st.warning(
            "⚠️ Architecture Notice: QR matrices have a definitive absolute upper limit storage footprint. "
            "Uploaded assets will automatically be optimized and scaled to fit inside the matrix architecture."
        )
    ...
    if is_pipeline_ready:
        ...
        try:
            ...
            with out_col2:
                ...
                st.info("🔹 **Error Mitigation Threshold:** Level L (Low Standard)")
            ...
            st.error(f"🚨 Unexpected QR Engine Failure: {str(e)}")
    else:
        st.info("💡 Provide text, a URL, or an image payload to generate a QR matrix.")
